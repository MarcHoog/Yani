...
test('leaves absolute links to the browser', async () => {
  const onNavigate = vi.fn()
  const screen = await render(<Sidebar brand="Yani" groups={[{ items: [{ label: 'Docs', href: 'https://example.test/docs' }] }]} onNavigate={onNavigate} />)
  ...
  screen.getByRole('link', { name: 'Docs' }).element().dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }))
